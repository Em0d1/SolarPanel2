...
st.sett_page_config(page_title="Solar Panel Efficiency Calculator")

st.title("Solar Panel Efficiency Calculator")
st.markdown("Estimate your solar energy output using real-time weather data.")

# --- Inputs ---
city = st.text.input("Enter your city", "Toronto")
api_key = st.text_input("OpenWeatherMap API Key", type="password")
panel_power = st.number_input("Panel Wattage (W)", min_value=100, max_value=600, value=300)
...
def get_coordinates(city, api_key):
  ...
  def calculate_output(irradiance, panel_power, num_panels, efficiency, sunlight_hours):
    total_kw = (panel_power * num_pnaels) / 1000
  ...
  if city and api_key:
    ...
    if lat and lon:
      ...
      if weather_data:
        ...
        st.write(f"**Esstimated Co2 saved/day:** {co2_saved:.2f} kg")
